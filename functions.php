<?php
	...
	/* OpenBookings.org - Copyright (C) 2005-2009 JÈrÙme ROGER (jerome@openbookings.org)
	...
	function FormatDate($date_a_verifier, $debutfin) {
		...
		if(strlen($date_a_verifier) <= 2 || ( strlen($date_a_verifier) >= 5  && strlen($date_a_verifier) <= 7)) { //pour prendre 6 caractËres ex: 1/2005

			// SÈlection de l'annÈe
			...
			$timestamp = strtotime($annee . "-01-01"); // timestamp du premier janvier de l'annÈe choisie

			while(intval(date("W",$timestamp)) != 1) { $timestamp = strtotime("+1 days", $timestamp); } // Avancer jusqu'‡ la semaine 1 (annÈe-01-01 peut Ítre en semaine 53)

			while(intval(date("W", $timestamp)) != $date_a_verifier) { $timestamp = strtotime("+1 weeks", $timestamp); } // avancer semaine par semaine jusqu'‡ la semaine spÈcifiÈe par l'utilisateur
		...
		//modif annÈe en 31/12/2005 ou 31/12/05 :
	...
	function validateString($target, $string) { // [] () / . ? ! _ - &aa[aaaa]; &000;
		...
		switch($target) {
			...
			case "sql": $pattern = "#^([…» À‹€Œ‘ƒœ÷ƒ≈«A-ZÈËÎÍ¸˚ÁÓÙ‚Ôˆ‰Âaa-z0-9\.\?\!\[\]\(\)\s'/_-]|&[a-z]{2,6};|&\#[0-9]{3,3};){1,}$#"; break;
